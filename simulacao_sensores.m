...
% %%%%%versão nova
    for k=1:length(angs) %para cada um dos sensores
        Vsx = round(linspace(pos_sensor(1,k),s_i(1,k),janela));
        Vsy = round(linspace(pos_sensor(2,k),s_i(2,k),janela));
        Vs = [janela+Vsx ; janela+Vsy];
        vv = AA((Vs(1,:)-1)*(ymaxA+2*janela) + Vs(2,:));
        [valorr idc] = find(vv~=255);
        if ~isempty(idc)
            s_i(1,k) = Vsx(idc(1));
            s_i(2,k) = Vsy(idc(1));
        end
    ...
    if ds0(1) <= 2 %colisão de pior caso com
        colidiu = 1;
    end
